 ...
 *
 * obsidian-project-intake (AC1, AC2, AC3, AC5, AC6, AC7 — dritte "Neues Projekt"-Option, S-249):
 *   AC1  — Dritte Option „Aus Obsidian-Notizen" neben den zwei bestehenden ("+ Neues Repo" /
 *          "Klonen"-Button je Zeile, beide UNVERÄNDERT). Kein Vault konfiguriert
 *          (GET /api/settings/obsidian-vault-path → configured:false) → Toggle-Button
 *          sichtbar aber disabled + Text-Hinweis „Zuerst Obsidian-Vault in den
 *          Einstellungen setzen" (nie nur Farbe); Touch-Target ≥ 44 px.
 *   AC2  — Auswahl lädt GET /api/settings/obsidian-vault/projects → auswählbare Liste
 *          (name sichtbar, path wird weitergereicht). Leere Liste → Hinweis; Ladefehler →
 *          sichtbare Fehleranzeige, kein Crash.
 *   AC3  — „Auslösen" POSTet GENAU EINMAL { command: '/agent-flow:from-notes <path>' } an
 *          POST /api/command; <path> ausschließlich aus der Liste (kein Freitext), zu
 *          einer Zeile ohne Steuerzeichen kollabiert. Ohne Auswahl disabled; Doppelklick
 *          während „starting" → kein zweiter POST (Button gesperrt).
 *   AC5  — GET /api/session → state:"busy" → „Auslösen" disabled + Text-/Lock-Hinweis;
 *          Klick löst keinen POST aus (Busy-Poll-Muster wie TriggerPanel).
 *   AC6  — 202 → onNavigate('factory'), kein stehengebliebenes „gestartet"-Element.
 *   AC7  — 409 → sichtbare Fehleranzeige (Job läuft bereits), kein onNavigate. 400/500/
 *          Netzwerkfehler → Fehleranzeige mit Reset, kein onNavigate, kein Crash.
 ...
 * AC1: bestimmt, ob die dritte "Neues Projekt"-Option aktivierbar ist.
 * Fail-closed: jeder Fehler (Netzwerk/HTTP) → configured:false (Toggle bleibt
 * deaktiviert statt einen unbestätigten Vault-Zustand zu riskieren).
 *
 * @param {typeof fetch} fetchImpl
 * @returns {Promise<{ configured: boolean }>}
 */
async function fetchObsidianVaultStatus(fetchImpl) {
  try {
    const fn = fetchImpl ?? globalThis.fetch.bind(globalThis);
 ...
 */
function ObsidianImportSection({ fetchFn, onNavigate }) {
 ...
 * obsidian-project-intake AC1 (S-249): fetches GET /api/settings/obsidian-vault-path once
 *      on mount to determine whether the third "Aus Obsidian-Notizen" option is enabled;
 *      renders the toggle unconditionally (disabled + text hint when not configured).
 *
 * @param {{ fetchFn?: typeof fetch, onNavigate?: (view: string) => void }} props
 */
function RepoList({ fetchFn, onNavigate }) {
  ...
  // obsidian-project-intake AC1 (S-249): 'loading' | 'configured' | 'unconfigured'
  const [obsidianVaultState, setObsidianVaultState] = useState('loading');
  ...
  // obsidian-project-intake AC1 (S-249): fetch vault-configured status once on mount.
  useEffect(() => {
    let cancelled = false;
    fetchObsidianVaultStatus(fetchFnRef.current).then(({ configured }) => {
      if (!cancelled) setObsidianVaultState(configured ? 'configured' : 'unconfigured');
    });
    return () => { cancelled = true; };
  }, []);

  const obsidianConfigured = obsidianVaultState === 'configured';

  return (
    <>
      <section style={styles.section} aria-labelledby="repo-list-heading">
        {/* AC4 anchor: Neues Repo — über der Liste; obsidian-project-intake AC1: dritte Option */}
        <div style={styles.listHeader}>
          ...
          <div style={styles.listHeaderActions}>
            ...
            {/* obsidian-project-intake AC1: dritte Option, disabled + Text-Hinweis wenn kein Vault konfiguriert */}
            <div style={styles.obsidianToggleWrapper}>
              <button
                type="button"
                style={obsidianConfigured ? styles.btnObsidianToggle : styles.btnObsidianToggleDisabled}
                disabled={!obsidianConfigured}
                onClick={() => setShowObsidianImport((v) => !v)}
                aria-expanded={showObsidianImport}
                aria-controls="obsidian-import-section"
                aria-describedby={!obsidianConfigured ? 'obsidian-toggle-disabled-hint' : undefined}
                aria-label={
                  !obsidianConfigured
                    ? 'Aus Obsidian-Notizen — deaktiviert, zuerst Vault in den Einstellungen setzen'
                    : showObsidianImport
                    ? 'Obsidian-Import-Formular schließen'
                    : 'Aus Obsidian-Notizen — neues Projekt anlegen'
                }
              >
                {showObsidianImport ? 'Formular schließen' : 'Aus Obsidian-Notizen'}
              </button>
              {/* AC1: Text-Hinweis, nie nur Farbe — nur wenn Status bekannt+nicht konfiguriert */}
              {obsidianVaultState === 'unconfigured' && (
                <span id="obsidian-toggle-disabled-hint" style={styles.obsidianDisabledHint}>
                  Zuerst Obsidian-Vault in den Einstellungen setzen
                </span>
              )}
            </div>
          </div>
        ...
        {/* obsidian-project-intake AC2/AC3/AC5/AC6/AC7: Import-Sektion ein-/ausblendbar */}
        <div id="obsidian-import-section" hidden={!showObsidianImport}>
          {showObsidianImport && (
  ...
  return (
    <main style={styles.view} aria-label="GitHub-Ansicht">
      <div style={styles.inner}>
        ...
        {/* AC3, AC4, AC6 — Repo-Liste mit Andockpunkten; obsidian-project-intake AC1-AC7 — dritte Option */}
        <RepoList fetchFn={fetchFn} onNavigate={onNavigate} />
...
const styles = {
  ...
  // obsidian-project-intake AC3/AC5: disabled-Zustand des "Auslösen"-Buttons
  btnPrimaryDisabled: {
    padding: '10px 20px',
    background: '#1e293b',
    color: '#64748b',
  ...
  // obsidian-project-intake AC1 (S-249): Container für die beiden Toggle-Buttons
  listHeaderActions: {
    display: 'flex',
    alignItems: 'center',
    gap: 12,
  ...
  // obsidian-project-intake AC1: Wrapper für Toggle-Button + Disabled-Hinweis (vertikal gestapelt)
  obsidianToggleWrapper: {
    display: 'flex',
    flexDirection: 'column',
    gap: 4,
  },
  btnObsidianToggle: {
    padding: '8px 16px',
    background: '#7c3aed',    // Kontrast #fff/#7c3aed ≥ 4.5:1
    color: '#ffffff',
    border: 'none',
    borderRadius: 4,
    fontSize: 13,
  ...
  // AC1: Text-Hinweis bei deaktivierter Option — nie nur Farbe
  obsidianDisabledHint: {
    fontSize: 11,
  ...
  // AC5: Busy-Lock-Hinweis — Text-Label, nicht nur Farbe
  obsidianLockNotice: {
    padding: '8px 10px',
    background: '#1a1500',

...
- **Workspace (Dateisystem)** — konfigurierbarer `WORKSPACE_DIR` (Env) auf dem persistenten Volume, uid-1000-schreibbar, beim Boot idempotent angelegt: Ziel lokaler Repo-Klone ([[github-repo-clone]]) und Quelle der Workspace-Übersicht ([[workspace-repos]]: Listing/Pull/Löschen). Alle Klon-/Pull-/Lösch-Ziele strikt innerhalb `WORKSPACE_DIR`.

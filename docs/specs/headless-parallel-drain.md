...
1. **Interaktiver Pfad bleibt 100% unangetastet.** Der manuelle Knopf „Board abarbeiten" (S-196, [[taktgeber-nachtwaechter]] AC12) und das Terminal laufen **weiterhin rein über `CommandService`/PTY** (kein `claude -p`, kein PTY-Lock-Bypass). Weder `CommandService` noch der globale `JobLock` werden umgebaut — der verworfene Alt-Ansatz (interaktiven Lock projektweise entkoppeln) wird **nicht** gebaut.
   ...
   > **⟶ Superseded für den manuellen Knopf (2026-07-01, [[headless-manual-drain]] / ADR-017):** Der **manuelle** „Board abarbeiten"-Knopf (S-196) — hier und in AC6 ursprünglich als rein interaktiv/PTY beschrieben — läuft seither **ebenfalls headless** (eigene `ProjectDrain`/`HeadlessFlowRunner`-Instanz + eigenes `ProjectJobLock`, `claude -p '/agent-flow:flow …'` je Drain-Runde, optionaler Cost-Mode). Diese „Interaktiver-Pfad-bleibt-unangetastet"-Abgrenzung gilt seit dann nur noch für das **Terminal** + die verschlankte Befehls-Auslösung (`adopt`/`preview`/`train`/`new-project` + Kill-Switch), **nicht** mehr für den Board-Drain-Knopf. Alle „manueller Knopf = rein interaktiv"-Aussagen dieser Spec sind entsprechend superseded.
...
- **AC1** — `HeadlessFlowRunner` startet einen `claude -p`-Kindprozess mit **konfigurierbarem** Befehl (Default-argv `['-p','/agent-flow:flow','--dangerously-skip-permissions']`, optional weitere Args wie `--cost <mode>`) in **Array-argv-Form** (kein Shell-String, security/R03), `cwd` = aufgelöster, validierter Projekt-Pfad. Übernimmt 1:1 aus [[headless-reconcile-runner]]: Env-Allowlist + `CLAUDE_CODE_OAUTH_TOKEN` (`buildChildEnv`), harter `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`-Block, 401-Vorrang vor Exit 0 (`isAuthError`), `close`-Event als **einzige** Fertig-Quelle, projektweises `ProjectJobLock` (finally-Freigabe), In-Memory-Job-Registry `getJob(jobId) → {status:'running'|'done'|'failed'|'auth-expired', result?, error?}`. *(3,4)*
  - **⟶ Ergänzt (2026-07-01, [[headless-manual-drain]] / ADR-017):** Dieser `HeadlessFlowRunner`-Baustein trägt seither **auch** den **manuellen** „Board abarbeiten"-Drain (eigene Runner- + `ProjectJobLock`-Instanz, getrennt vom Nacht-Drain) — nicht mehr nur den automatischen Nacht-Drain; die frühere Annahme, der manuelle Pfad bleibe rein interaktiv, ist damit superseded.
...
- **AC6** — Ohne injizierten Runner (Default) verhält sich `ProjectDrain` **bit-identisch** zu heute (interaktiver Adapter): das **Terminal** und die verschlankte Befehls-Auslösung bleiben rein `CommandService`/PTY — **kein** Verhaltens-/Regressions-Unterschied im interaktiven Pfad. *(1,8)*
  - **⟶ Superseded für den manuellen „Board abarbeiten"-Knopf (2026-07-01, [[headless-manual-drain]] / ADR-017):** dieser Knopf verdrahtet seither den **headless** Adapter (eigene `ProjectDrain`/`HeadlessFlowRunner`-Instanz + eigenes `ProjectJobLock`) und läuft **nicht** mehr über den Default-/interaktiven Pfad. Der Default-`ProjectDrain` ohne injizierten Runner bleibt (uninjiziert) unverändert interaktiv — nur bindet der manuelle Knopf jetzt den injizierten headless Adapter.
...
- **Server-Neustart** — In-Memory-Job-Registry geht verloren (Nicht-Ziel: keine persistente Job-Historie); ein laufender Subprozess wird ggf. verwaist (Timeout/OS bereinigt) — akzeptiert, im Restrisiko vermerkt. **Superseded/ergänzt (ADR-020, 2026-07-03):** die `DrainJobRegistry` ist seither datei-basiert persistiert und wird beim Boot über `reconcileOrphans()` + `BootDrainRecovery` idempotent wiederangelaufen — der Registry-Eintrag geht nicht mehr verloren, siehe [[drain-restart-robustness]] (AC1–AC8). Der verwaiste Kindprozess selbst überlebt weiterhin nicht (bewusste Nicht-Entscheidung gegen detached-Spawn, [[drain-restart-robustness]] AC10).
...
- **Keine** persistente headless-Job-Historie (In-Memory-Registry, geht bei Neustart verloren). **Superseded/ergänzt (ADR-020, 2026-07-03):** die Registry ist seither datei-basiert persistiert + Boot-Wiederanlauf verdrahtet, siehe [[drain-restart-robustness]] (AC1–AC8) — bewusst weiterhin **keine** unbegrenzte Historie (kleine Betreiber-nahe Beobachtbarkeits-Ablage, [[drain-restart-robustness]] Nicht-Ziele).

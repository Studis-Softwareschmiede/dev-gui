...
- **A1 (Browser-Bereitstellung — ENTSCHIEDEN, Owner 2026-07-26):** Die Browser sind bereits **fest im dev-gui-Image** eingebaut (Dockerfile: `npx playwright install --with-deps chromium`, `PLAYWRIGHT_BROWSERS_PATH=/ms-playwright`, Chromium-Build passend zur image-eigenen `@playwright/test`-Version — Bestandsaufnahme 2026-07-26: 1.61.1 / chromium-1228). **Kein Laufzeit-Nachladen.** Damit das trägt, wird die Playwright-Version **zentral gepinnt**: das agent-flow-Regressions-Template und die Projekt-Repos (Referenzfall flashrescue: bisher `"latest"`) pinnen dieselbe Version wie das dev-gui-Image. Versions-Gleichheit ist damit Konvention der Fabrik, nicht Zufall. Der ursprünglich vorgesehene ADR entfällt zugunsten dieser Owner-Entscheidung; ein kurzer ADR-Eintrag, der sie festhält, genügt. AC2 fordert weiterhin das beobachtbare Ergebnis („Browser verfügbar oder klarer Vorbedingungsfehler").
...
- **AC2** — **Browser verfügbar (A1 entschieden — fest im Image):** Die Playwright-Browser (`chromium`) liegen fest im dev-gui-Image (`/ms-playwright`, Bestand) — **kein** Laufzeit-Nachladen, **kein** ad-hoc-npx pro Lauf. Der Runner prüft vor dem Test-Start, dass die im Klon installierte `@playwright/test`-Version zu einem vorhandenen Browser-Build passt (Versions-Guard); passt sie nicht → terminaler **`precondition-error`** mit secret-freier Diagnose „Playwright-Version des Projekts passt nicht zum Image-Browser" ([[regression-run]] AC10) statt kryptischem Playwright-Launch-Absturz. Flankierend (A1): agent-flow-Template + Projekt-Repos pinnen die Image-Version zentral, so dass dieser Guard im Normalbetrieb nie anschlägt.
...
- **AC9** — **Test-Config provisioniert:** Das frisch ausgerollte/erreichbare Testobjekt ist mit der für den Referenzfall nötigen Test-Config provisioniert (z.B. `fee.prozent: 0` für den Freundschaftsfall), sodass die Suite grün werden **kann**. Mechanik projekt-/deployment-spezifisch (A3, `architekt`-Detail) — AC fordert nur das beobachtbare Ergebnis. **Präzisierung (S-418, umgesetzte Mechanik):** der Runner erfindet KEIN eigenes Provisionierungs-Schema/keinen eigenen Mount — sein Beitrag ist der volle, ungefilterte Env-Durchgriff in den Playwright-Kindprozess (`{ ...process.env, ...secretEnv }`, unverändert seit AC7/AC8), der Leitkanal, über den die bereits im Referenzprojekt vorhandene, suite-eigene Provisionierung greifen kann (flashrescue: `_canvas-helpers.ts#setFeeViaSettings()` schreibt `settings.json` in das über `FLASHRESCUE_CONFIG_DIR` bestimmte, lokal gemountete Config-Verzeichnis — Mechanismus/Mount-Bereitstellung bleiben Deployment-Voraussetzung, außerhalb des `RegressionRunner`-Scopes).
- **AC10** — **REAL_SEND nie automatisch scharf:** Der Runner setzt `FLASHRESCUE_REGRESSION_ALLOW_REAL_SEND` (bzw. das generische REAL_SEND-Gate) **nie**; die dahinter liegenden Schritte skippen kontrolliert, der Rest der Suite muss grün werden können. **Präzisierung (S-418):** `defaultRunPlaywright` entfernt jede bekannte REAL_SEND-Gate-Env-Var (`REAL_SEND_GATE_ENV_VARS`) hart aus der Kind-Env — auch wenn sie zufällig im geerbten `process.env` (Operator-Shell) oder im projekt-eigenen `.env.gpg` steht.
...
- `docs/architecture.md` — kurzer ADR-Eintrag, der die Owner-Entscheidung A1 festhält (Browser fest im Image + zentrales Versions-Pinning; entschieden 2026-07-26, kein offener Entwurf mehr).

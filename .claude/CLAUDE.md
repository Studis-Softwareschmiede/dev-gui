...
- **Engine:** fernsteuert eine INTERAKTIVE Claude-Code-Session (Abo-OAuth) per `node-pty` — KEIN Anthropic-API. `claude -p` ist **projektweit erlaubt** (Owner-Entscheidung 2026-07-01, ADR-016 in `docs/architecture.md`) — sowohl **zustandslos/tool-los** als auch **tool-fähig via `HeadlessFlowRunner`**; der **interaktive PTY-Pfad bleibt parallel bestehen** (kein PTY-Lock-Bypass für ihn) und ist keine Alleinstellungs-Doktrin mehr. **Bislang benannte Bausteine (headless, one-shot bzw. langlaufend, kein PTY-Lock, kein API-Key, auditiert):** (1) der zustandslose „Let Claude proof"-Helfer (`POST /api/assist/refine`, eigene `AssistService`-Boundary, tool-los) — AC11 docs/specs/fabric-intake-dialog.md; (2) der zustandslose Quellen-Such-Helfer (`POST /api/assist/knowledge-sources`, eigene `KnowledgeSourceService`-Boundary, **Web-fähig**: Tool-Allowlist exklusiv `WebSearch`, kein `WebFetch`) — AC10 docs/specs/team-knowledge-add.md; (3) der Headless-Reconcile-Runner (`POST /api/reconcile`, eigene `HeadlessReconcileRunner`-Boundary, `claude -p '/agent-flow:reconcile'` als Kindprozess mit eigenem `ProjectJobLock`, kein Idle-/Rate-Timer) — AC1–AC7 docs/specs/headless-reconcile-runner.md; (4) der **headless-Nacht-Drain** des Nachtwächters (`NightWatchScheduler` → separate, headless verdrahtete `ProjectDrain`-Instanz, `HeadlessFlowRunner`/`HeadlessFlowRunnerAdapter`, `claude -p '/agent-flow:flow'` als eigener Kindprozess je Lauf, **eigene** `ProjectJobLock`-Instanz — bewusst getrennt von der `ProjectDrain`-eigenen Session-Lock-Instanz, sonst Selbst-Blockade) — AC1–AC12 docs/specs/headless-parallel-drain.md; ermöglicht **echte Parallelität** mehrerer Projekt-Drains nachts, ohne den globalen PTY-Lock zu berühren; (5) der **Idee-Specify-Chat + Requirement-Finalizer** (`IdeaSpecifyChatService` — zustandsloser, tool-loser Multi-Turn-`claude -p`-Chat je Turn, Verlauf via stdin; `IdeaSpecifyFinalizer` — eigene `HeadlessFlowRunner`-Instanz mit eigener `ProjectJobLock`-Instanz, fährt `claude -p '/agent-flow:requirement'` als Kindprozess) — AC1–AC13 docs/specs/idea-specify-chat.md; (6) der **manuelle „Board abarbeiten"-Knopf** (Fabrik-Panel, „Arbeiten"-Tab) läuft seit **ADR-017** (Owner-Entscheidung 2026-07-01) **ebenfalls headless**: `POST /api/projects/:slug/drain` fährt eine **eigene, headless verdrahtete `ProjectDrain`-Instanz** (`HeadlessFlowRunnerAdapter` um eine **eigene `HeadlessFlowRunner`-Instanz** mit **eigener `ProjectJobLock`-Instanz** — getrennt von Nacht-Drain/Reconcile-Runner/`IdeaSpecifyFinalizer`), deren Flow-Schritt je Drain-Runde ein `claude -p '/agent-flow:flow …'`-Kindprozess ist (kein PTY-Write, kein globaler PTY-Lock); optionaler Cost-Mode via `--cost <mode>` — AC1–AC8 docs/specs/headless-manual-drain.md; (7) der **headless Auto-Retro-Runner** (`RetroAutoQueue` + `HeadlessRetroRunner`, `src/RetroAutoQueue.js` + `src/HeadlessRetroRunner.js`, S-256/S-257 gelandet) wird nach einem automatisch ausgelösten Board-Lauf eingereiht — die Auslöse-Naht ([[retro-auto-trigger]], S-259–S-261, **gelandet**) ist verdrahtet: `AutoRetroTrigger` (`src/AutoRetroTrigger.js`, `isRetroDue`) hängt an derselben Stelle wie [[drain-completion-report]] — **sowohl** am Nacht-Drain-Abschluss (`NightWatchScheduler`) **als auch** am manuellen Drain-Abschluss (`projectDrainRouter`), **geteilte** Instanz, kein zweiter Codepfad. Default bleibt **aus** (`RetroAutoSettingsStore` `enabled:false`) — ohne bewusste Aktivierung über den Schalter „Danach automatisch Retro durchführen" läuft **kein** Auto-Retro (heutiges Verhalten, Wochen-Cooldown G3 bleibt aktiv). Ist der Schalter an, prüft `isRetroDue` je Drain-Abschluss (Schalter an + `flowRuns≥1` + Repo-Dedup) und reiht bei Fälligkeit **best-effort/fire-and-forget** in die `RetroAutoQueue` ein: die `RetroAutoQueue` ist eine **global serielle FIFO-Warteschlange** — zu **keinem Zeitpunkt** läuft mehr als **ein** Retro-Lauf gleichzeitig (Repo-Dedup via `isPendingOrActive`, read-only `getStatus`), weil der Retro-Agent in eine **geteilte** Lern-Ablage schreibt (`LEARNINGS.md` + globale Knowledge-Packs, PR gegen agent-flow); ein fehlgeschlagener Lauf **stoppt die Queue nicht** (degradierend). Je Repo fährt der `HeadlessRetroRunner` einen `claude -p '/agent-flow:retro --force'`-Kindprozess (G3-Cooldown-Bypass) über eine **eigene** `HeadlessFlowRunner`-Instanz mit **eigener** `ProjectJobLock`-Instanz — getrennt von Nacht-Drain/manuellem Drain/Reconcile-Runner/`IdeaSpecifyFinalizer`/`CostModeModelCheck`; eigener, entkoppelter Timeout `RETRO_HEADLESS_TIMEOUT_MS`, `close`-Event als einzige Fertig-Quelle; **Per-Lauf-Audit** bei Start/Ende/Fehler (secret-frei) — AC1–AC7 docs/specs/retro-auto-queue.md, ADR-018; (8) der **headless Obsidian-Ingest-Runner** (`ObsidianIngestRunner`, `src/ObsidianIngestRunner.js`, S-250, Backend-Anteil von [[obsidian-question-catalog]] — das Overlay/UI ist S-251) fährt einen `claude -p '/agent-flow:from-notes <projectFolderPath>'`-Kindprozess über eine **eigene** `ProjectJobLock`-Instanz — getrennt von ALLEN anderen headless-Locks. Anders als die übrigen Runner endet EIN Lauf je Runde in `done` **oder** dem **Interrupt-Zustand** `needs-answers` (maschinenlesbarer Fragenkatalog `{stage,id,frage,quelle,optionen?,pflicht?}`); das Lock bleibt WÄHREND `needs-answers` gehalten (kein Doppel-Lauf) und wird erst bei einem terminalen Zustand freigegeben. `POST .../obsidian-ingest/:jobId/answers` reicht die gebündelten Antworten **via STDIN** (nie argv) in denselben `claude`-Session-Kontext zurück (`--resume <session-id>`) — Resume, kein neuer Lauf. `projectFolderPath` wird vault-confined gegen `listObsidianVaultProjects()` validiert (obsidian-vault-config AC5, kein Freitext-Pfad). Wiederverwendet dieselben `HeadlessRunnerCore.js`-Primitive (Env-Block/Audit/Lock-Disziplin) wie `HeadlessFlowRunner` — die state machine (Interrupt/Resume) ist der einzige neue Baustein, da `HeadlessFlowRunner` fire-and-forget bis `done` läuft und dafür keine Naht bietet. **Rein interaktiv (PTY-`CommandService`, unverändert)** bleiben nur noch das **Terminal** (freies Arbeiten) und die **verschlankte Befehls-Auslösung** (`adopt`/`preview`/`train`/`new-project` + Kill-Switch); der interaktive PTY-Pfad bleibt vollständig bestehen — nur der **Ausführungspfad des „Board abarbeiten"-Knopfs** wechselte von interaktiv auf headless. Alle acht Boundaries sind getrennt (unterschiedliche Capability + Risikoprofil); `AssistService` bleibt tool-/netz-los. Kein Anthropic-API bleibt unverändert ausgeschlossen.

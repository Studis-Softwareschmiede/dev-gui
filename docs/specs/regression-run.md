...
## Alternative Flows
- **A3 — `ephemeral-infra`:** der Lauf startet Playwright OHNE lokale Erreichbarkeitsprüfung/Frisch-Ausrollen (beides AC5/AC7-exklusiv für `local`); der Dialog hat vorab den Kosten-/Ressourcen-Hinweis gezeigt. **Umsetzungsstand (2026-07-18): gebaut (S-362).** Provisionierung/Abbau des `rtest-*`-Wegwerf-Ziels selbst lebt — verbindlich gemäß agent-flow `regression-playwright-conventions` AC4 — **fixture-scoped INNERHALB der einzelnen Playwright-Suite** (`try`/`finally` je Test, s. `tests/regression/vps/*.spec.ts`), NICHT im Runner (dev-gui definiert die Infra-Leitplanken/`target`-Semantik nicht neu, Spec-Nicht-Ziel). Der Runner setzt `REGRESSION_BASE_URL` analog zum `local`-Pfad (dieselbe Port-Auflösung), weil bestehende `ephemeral-infra`-Suiten über die lokal laufende Applikation navigieren. ZUSÄTZLICH zum In-Test-Teardown garantiert der Runner ein best-effort Sicherheitsnetz (agent-flow `regression-runner` AC7/AC8): nach JEDEM Lauf-Ausgang (passed/failed/error/Timeout/interner Fehler) fegt er über alle konfigurierten VPS-Provider und löscht jede Maschine mit `rtest-*`-Namenspräfix — greift, wenn der Runner-eigene Runaway-Timeout den Playwright-Kindprozess killt, bevor dessen eigener Fixture-Teardown lief. Scheitert die Provisionierung innerhalb der Suite, ist das ein normaler Playwright-Testfehlschlag (CTRF `failed`), kein Runner-`error` — der Runner selbst provisioniert nichts, das fehlschlagen könnte.
...
### `ephemeral-infra`-Ausführungspfad (A3 — S-362, gebaut)
- **AC12** — Bei `target: ephemeral-infra` startet der Lauf die Suite (OHNE lokale Erreichbarkeitsprüfung/Frisch-Ausrollen, s. AC5/AC11) und garantiert danach ein Cleanup — auch im Fehler-, Timeout- und Abbruch-Pfad (kein verwaistes, kostenpflichtiges Ziel). Namensschema (`rtest-*`), Produktiv-Allowlist und Cleanup-Garantie folgen agent-flow `regression-runner` AC4/AC7/AC8; dev-gui **spiegelt** sie, definiert sie **nicht** neu — konkret: agent-flow `regression-playwright-conventions` AC4 legt das Provisionier-/Teardown-Fixture-Muster für Infra-Ketten **verbindlich auf die einzelne Playwright-Suite** (fixture-scoped `try`/`finally`, s. `tests/regression/vps/*.spec.ts`, die ihr eigenes `rtest-*`-Ziel über die dev-gui-eigene UI anlegt/abbaut), **nicht** auf den Runner. Der Runner selbst provisioniert daher **kein** Ziel vorab — „Scheitert die Provisionierung" ist folglich ein normaler Playwright-Testfehlschlag (CTRF `status:"failed"`), kein Runner-`error`. Der Runner trägt **zusätzlich** zum In-Test-Fixture-Teardown ein best-effort Sicherheitsnetz bei: nach **jedem** Lauf-Ausgang (`passed`/`failed`/`error`/Timeout/interner Fehler) fegt er über alle konfigurierten VPS-Provider und löscht **jede** Maschine mit `rtest-*`-Namenspräfix (Produktiv-Allowlist: ausschließlich `rtest-*`, nie andere Namen) — greift, wenn der Runner-eigene Runaway-Timeout den Playwright-Kindprozess killt, bevor dessen eigener Fixture-Teardown lief. Der Kosten-/Ressourcen-Hinweis ist dem Owner vorab gezeigt worden (AC6). Bekannte Grenze: der Sweep ist global (alle `rtest-*`-Maschinen dieses Servers) — bei echter Parallelität mehrerer Projekte mit je eigenen `ephemeral-infra`-Läufen könnte er eine fremde, noch laufende Maschine treffen; heute existieren `ephemeral-infra`-Suiten nur in diesem Repo (kein praktischer Kollisionsfall). Ein Absturz des gesamten dev-gui-Serverprozesses bleibt außerhalb der Reichweite jedes In-Process-Sicherheitsnetzes (bräuchte eine separate Reconcile-Instanz, Nicht-Ziel dieser Story).

### Scope-Vertrag Reader↔Runner konsistent (Befund 2026-07-22)
- **AC13** — **`verbund`-Scope ohne `id` ausführbar:** Für `typ: "verbund"` ist `id` **optional**. `validateScope` akzeptiert `{ typ: "verbund" }` **ohne** `id` als gültig (kein `400 "invalid"`/`missing-id`); nur `typ: "bereich"` verlangt weiterhin ein nicht-leeres `id`, `typ: "gesamt"` ignoriert `id` (Bestandsverhalten). **Begründung (bindend):** `scopeToTestPath` **und** `#resolveTarget` ignorieren die `verbund`-`id` ohnehin — alle Verbund-Suiten laufen gemeinsam unter `tests/regression/verbund` (Layout-Vertrag agent-flow `regression-playwright-conventions`); die `id`-Pflicht war für `verbund` funktionslos. Der Reader (`readRegressionSuites`, `RegressionSuiteReader.js`) listet den Verbund-Eintrag bereits als `scope: { typ: "verbund" }` **ohne** `id`, und der Ausführen-Dialog (`RegressionRunDialog.jsx`) sendet **exakt** dieses Objekt — Reader, Dialog, Router und Runner behandeln das `verbund`-Scope damit **einheitlich**. Ein „Verbund ausführen"-Klick wird **nicht mehr** mit `400 "Invalid scope"`/`missing-id` abgelehnt. Wird für `verbund` doch ein `id` mitgesendet, ist das **tolerant** (kein Fehler) — es bleibt wirkungslos (Bestandsverhalten `scopeToTestPath`/`#resolveTarget`).

- **AC11** — **Testobjekt-Weiche (`target`) vor dem Lauf:** der Runner liest das je Suite deklarierte `target` aus der Begleitbeschreibung (**dieselbe** Lese-Boundary wie der Ausführen-Dialog, AC4 — kein zweiter Parser) und verzweigt: `local` → local-Pfad (AC5/AC7/AC8); `ephemeral-infra` → **A3/AC12** (gebaut); `url` → **A4** (weiterhin nicht gebaut). Ein Testobjekt, dessen Ausführungspfad **noch nicht gebaut** ist (aktuell nur `url`), endet **sofort** als `error` mit dem Grund „Testobjekt `<target>` wird noch nicht unterstützt" — persistiert nach AC10, **ohne** Playwright-Start und **ohne** die `local`-Erreichbarkeitsprüfung (AC5: `ephemeral-infra`/`url` durchlaufen sie nicht). Ein Fallback auf den `local`-Pfad ist **ausgeschlossen** — er prüfte das falsche Testobjekt und meldete einen irreführenden Vorbedingungs-Fehler (verifizierter Befund 2026-07-08). Ist **kein** `target` deklariert (fehlende/unlesbare Begleitbeschreibung), gilt konservativ `local`. Ein **deklarierter, aber unbekannter/fehlerhafter** `target`-Wert (weder `local` noch `ephemeral-infra`/`url`, z. B. ein Tippfehler in der Begleitbeschreibung) zählt **nicht** als „kein target deklariert" — er ist bewusst deklariert, nur nicht erkannt — und durchläuft daher **denselben** Nicht-unterstützt-Zweig wie `url` (kein lokaler Fallback, s. o.); die Meldung übernimmt dabei **nur** die bekannten Werte wörtlich (Datenhygiene, der Wert kommt aus einer Repo-Datei, die der Runner nicht kontrolliert) — ein unbekannter Rohwert erzeugt eine generische Meldung „Testobjekt wird noch nicht unterstützt" statt des Wertes selbst. Der `gesamt`-Scope mischt Testobjekte: er läuft über den `local`-Pfad (Bestandsverhalten), unverändert.

## Verträge
- **POST `/api/projects/:slug/regression-run`** — Body `{ scope: { typ: "bereich"|"verbund"|"gesamt", id?: <bereich-id> }, freshRollout?: boolean }` — `id` ist **nur** für `typ: "bereich"` erforderlich; für `verbund`/`gesamt` optional bzw. wirkungslos (AC13, `validateScope`) → `{ runId, status: "running" }` · `409 { error: "busy" }` (Drain/Lauf aktiv) · `403` (keine Berechtigung). Die local-Erreichbarkeitsprüfung (AC5) läuft **asynchron als Teil des gestarteten Laufs** (Main Success Scenario Schritt 4–6), NICHT synchron im POST-Response — ein nicht erreichbares `local`-Ziel liefert daher **keinen** eigenen POST-Statuscode, sondern den Lauf-Zustand `precondition-error` über den GET-Endpunkt (Präzisierung: eine frühere Fassung nannte hier zusätzlich einen synchronen `422`-Code, der der beschriebenen Async-Ausführung widersprach).
...
- Suite deklariert ein noch nicht gebautes Testobjekt (`url`, A4) → sofortiger `error` „Testobjekt `<target>` wird noch nicht unterstützt" (AC11), persistiert nach AC10; **kein** stiller `local`-Fallback.
- `ephemeral-infra`-Sicherheitsnetz-Sweep (AC12) ohne konfigurierte VPS-Provider/Registry → best-effort übersprungen (kein Crash, degradiert auf den In-Test-Fixture-Teardown allein — keine zweite Absicherung).
- `ephemeral-infra`-Sicherheitsnetz-Sweep listet/löscht eine Maschine nicht (Provider-API-Fehler) → Fehler wird protokolliert (secret-frei), der Sweep macht mit der nächsten Maschine weiter, der Lauf-Abschluss selbst scheitert nie daran (best-effort).

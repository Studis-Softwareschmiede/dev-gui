...
- **`VpsMachine` (Read-Model, provider-agnostisch):** `{ provider, serverId, name, status, ipv4?, ipv6?, region?, serverType?, createdAt? }`. `status` normalisiert auf `running | stopped | provisioning | error | unknown`. **Kanonisches Schema + Normalisierungsregeln: [[data-model]]** (fehlende Felder → `null`/`unknown`, nie Fehler; fixiert in `docs/architecture.md` ADR-009). **IONOS-Präzisierung:** IONOS-Server sind unter Datacenters genestet; `serverId` für IONOS-Server ist ein zusammengesetzter String `"<datacenterId>/<serverId>"`, der beide Teile für Lifecycle-Operationen (start/stop) kodiert. Weder `/` noch andere Trennzeichen kommen in IONOS-UUIDs vor — das erste `/` trennt Datacenter von Server.
...
- **POST `/api/vps/machines/{provider}/{serverId}/start`** / **`…/stop`** → `{ result: "ok"|"unsupported"|"error", reason? }`. **IONOS-Routing-Präzisierung:** Da IONOS composite serverIds ein `/` enthalten, verwendet der Router einen Express-5-`*splat`-Param, der die Pfadsegmente zwischen `{provider}` und `/start`|`/stop` als Array auffängt und mit `/` zusammensetzt. Clients senden die composite ID als literal Pfadsegmente (z.B. `POST …/ionos/dc-uuid/srv-uuid/start`) ohne URL-Encoding. Clients für andere Provider (Hetzner, Hostinger) übergeben ihre einfache serverId wie bisher als ein Segment.

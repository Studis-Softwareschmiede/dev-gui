...
7. **Knopf je App.** In der Deploy-Ansicht existiert ein Knopf „Passphrase anlegen", der **denselben** Provisionierungs-Dienst mit **derselben** Idempotenz-Garantie aufruft (AC2). Für eine App mit bereits vorhandenem Item ist das Ergebnis `already-exists` (No-Op) — sichtbar quittiert, **kein** Überschreiben. **(v3, Owner 2026-07-18):** Der Knopf lebt seit dem Deployments-Unterbereich-Umbau ([[deployments-gpg-subview]]) im linken Unterbereich „GPG-Schlüssel", **kompakt je gewählter App (Dropdown)** statt als Liste aller Apps — Dienst/Endpunkt/Idempotenz-Garantie **unverändert**, nur der Ort/die Darstellung.
...
### Read-only Existenz-Abfrage (v3, Owner 2026-07-18)
10. **Existenz ja/nein, ohne Wert.** Ein **read-only** Endpunkt beantwortet, ob für eine App bereits ein Bitwarden-Item `env.gpg-passphrase-<app>` existiert — **ausschließlich** als Boolean `exists`, **nie** mit einem Passphrasen-Wert. Er nutzt denselben `itemExists`-Pfad (bw get) wie der Provisionierungs-Dienst, mutiert nichts und legt nichts an. Die UI ([[deployments-gpg-subview]] AC5) gated damit den „Passphrase anlegen"-Knopf: existiert das Item, ist der Knopf deaktiviert. Ist der Bitwarden-Deploy-Zugang **nicht** `ready`, ist die Existenz **unbekannt** — der Endpunkt meldet dies geheimnisfrei (`reason: 'access-not-ready'`), ohne zu raten.
...
- **AC7** — In der Deploy-Ansicht löst der Knopf „Passphrase anlegen" die Nach-Provisionierung über **denselben** Dienst (AC2/AC3-Garantien) aus; eine bereits provisionierte App liefert `already-exists` (No-Op, quittiert), **kein** Überschreiben. **(v3):** Der Knopf lebt im Unterbereich „GPG-Schlüssel" ([[deployments-gpg-subview]]) kompakt je gewählter App (Dropdown); Dienst/Endpunkt/Idempotenz-Garantie unverändert, nur Ort/Darstellung. (Testbar: Knopf → Endpunkt → Dienst; vorhandenes Item → `already-exists`.)
...
- **AC16 (v3, Owner 2026-07-18)** — Eine **read-only** Existenz-Abfrage `GET /api/deployments/:app/gpg-exists` liefert **ausschließlich** `{ exists: boolean }` (bei nicht-`ready`-Zugang zusätzlich `reason: 'access-not-ready'`, `exists` dann `false`/`null`), **nie** einen Passphrasen-Wert; sie liegt hinter `AccessGuard` **+** `CRED_ADMIN_EMAILS`-Logik (ohne Berechtigung `403`, **kein** `bw`-Wert), mutiert nichts und legt nichts an (nutzt denselben `itemExists`-Pfad wie `provision()`). (Testbar: vorhandenes Item → `exists:true`; fehlendes Item → `exists:false`; Response enthält **nie** die Passphrase; unberechtigt → `403`, kein Item wird angelegt; Zugang nicht `ready` → `reason:'access-not-ready'`, kein Raten.)
...
- **Existenz-Abfrage-Endpunkt (HTTP, read-only, v3):** `GET /api/deployments/:app/gpg-exists` — hinter `AccessGuard` + `CRED_ADMIN_EMAILS`. Response geheimnisfrei: `{ exists: boolean, reason?: 'access-not-ready' }` — **nie** ein Passphrasen-Wert. Mutiert nichts (kein Audit-First-Zwang, da keine Mutation; der Guard genügt). Slug-Validierung wie der Provision-Endpunkt.
- **Provisionierungs-Dienst (read-only Ergänzung, v3):** eine read-only Methode (z.B. `itemExistsFor(app, { identity }): Promise<{ exists: boolean, reason?: 'access-not-ready' }>`), die den bestehenden internen `itemExists`-Pfad (bw get) nutzt und **nie** einen Wert nach außen gibt; ist der Zugang nicht `ready`, meldet sie `access-not-ready` statt zu raten.
...
- [[deployments-gpg-subview]] (v3: Nach-Provisionierungs-Knopf + Existenz-Gating leben im Unterbereich „GPG-Schlüssel"; konsumiert AC7 + AC16).

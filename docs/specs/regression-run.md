...
## Verträge
- **POST `/api/projects/:slug/regression-run`** — Body `{ scope: { typ: "bereich"|"verbund"|"gesamt", id?: <bereich-id|verbund-name> }, freshRollout?: boolean }` → `{ runId, status: "running" }` · `409 { error: "busy" }` (Drain/Lauf aktiv) · `403` (keine Berechtigung) · `422 { error: "precondition", reason }` (local nicht erreichbar).

...
- `ProjectDrain.drainProject(projectPath, opts) → Ergebnis { stopped, reason: "no-drain-target"|"token-limit-stop"|"window-end"|"already-busy", flowRuns, escalated:[storyId] }`. (`reason: "no-drain-target"` = weder Drain-Ziel-Story noch ein durch Vorgänger ready-werdendes `To Do` vorhanden, AC2.)

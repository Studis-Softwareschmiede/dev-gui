/**
 * CloudflareView.test.jsx — Unit tests for the CloudflareView placeholder component.
 *
 * Covers (view-cloudflare spec AC1–AC3):
 *   - AC1: View renders title "Cloudflare" as a semantic heading (<h1>/<h2>);
 *          reachability via tile click and deep-link (#/cloudflare) is verified in AppShell.
...
/** Renders CloudflareView with a jest-mocked onNavigate. */
function renderView() {
...
// ── AC1 — Title "Cloudflare" as semantic heading ──────────────────────────────

describe('CloudflareView — AC1: Title', () => {
  it('renders a heading with text "Cloudflare"', () => {
  ...
  it('renders the Cloudflare main landmark', () => {
    const { getByRole } = renderView();
...
describe('CloudflareView — AC2: Placeholder, no backend call', () => {
  let originalFetch;
  let originalXHR;
  ...
  beforeEach(() => {
    ...
    globalThis.fetch = jest.fn(() =>
      Promise.reject(new Error('CloudflareView must not call fetch'))
    );

    // Spy on XMLHttpRequest — must not be opened
    originalXHR = globalThis.XMLHttpRequest;
    globalThis.XMLHttpRequest = jest.fn(function () {
      this.open  = jest.fn(() => { throw new Error('CloudflareView must not open XHR'); });
      this.send  = jest.fn(() => { throw new Error('CloudflareView must not send XHR'); });
    });
  });
  ...
  it('renders a "folgt" placeholder hint', () => {
    const { getByRole } = renderView();
    const main = getByRole('main', { name: /cloudflare-ansicht/i });
...
describe('CloudflareView — AC3: Navigation back to panel', () => {

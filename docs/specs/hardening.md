...
- **AC2** — Alle Laufzeit-Mounts/Pfade liegen unter `/home/node`: Claude-Login-Volume → `/home/node/.claude`, gpg.pass → `/home/node/.config/softwareschmiede/gpg.pass`, Plugin-Cache → `/home/node/.claude/plugins`. **Build + Plugin-Auto-Provisionierung (Entrypoint) + Auth-Kette (gpg→mint→gh) funktionieren weiterhin als Non-Root.** (BOOTSTRAP.md-Mounts entsprechend aktualisiert.) `/home/node/.claude` (+ `/home/node/.config`) werden im Image node-owned vor-angelegt (`mkdir -p` + `chown -R node:node` **vor** `USER node`), damit ein leeres named-Volume node-beschreibbar initialisiert wird (sonst EACCES beim Plugin-Install). **Claude CLI legt OAuth-Credentials in `$HOME/.claude.json` ab — eine Datei im HOME, nicht unter `.claude/`** — daher würde sie sonst auf der Overlay-FS landen und beim Restart verschwinden. Der Entrypoint löst das idempotent: regulär vorhandene `.claude.json` wird einmalig nach `.claude/state.json` (auf das Volume) verschoben und durch einen Symlink ersetzt; alle künftigen Claude-Writes auf `.claude.json` schreiben transitiv nach `state.json`. Testbar: nach `docker stop && docker rm && docker run` (gleiches Volume) bleibt `~/.claude.json` ein Symlink auf `.claude/state.json`, und einmal eingeloggte OAuth-Daten überleben den Recreate ohne erneuten Browser-Login.

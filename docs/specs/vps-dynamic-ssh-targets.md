...
>
> **v2 (2026-06, Live-Befund-Nachzug):** Der erste echte End-to-End-Deploy gegen einen nur-dynamisch angelegten VPS (`testdevgui`, serverId `142574176`, host `188.34.202.209`, Tunnel `devgui-testdevgui`) scheiterte mit **„Unbekannter VPS: testdevgui"**, obwohl er im Deploy-Dropdown erscheint. Ursache: v1/S-167 stellte das **Listing** (`GET /api/deployments/vps-targets`) auf die dynamische Quelle um, aber die **Mutations-/Auflösungs-Handler im selben `deploymentsRouter`** (Deploy POST, Undeploy DELETE, Deployments-Listing GET) lösen die `vpsId` weiterhin **nur** über die statische Env-Map auf (`vpsTargets.get(vpsId)`). v2 ergänzt **AC9**: diese drei Handler nutzen dieselbe vereinigte Quelle wie das Listing + `resolveVpsTarget`.
...
7. **Deploy-/Undeploy-/Deployments-Auflösung (v2):** Die `vpsId`-Auflösung in den **`deploymentsRouter`-Handlern** — **Deploy** (`POST /api/deployments`, heute `vpsTargets.get(vpsId)` an `src/deploymentsRouter.js:314`), **Undeploy** (`DELETE /api/deployments/:vps/:hostname`, Zeile 422) und **Deployments-Listing** (`GET /api/deployments`, Zeile 261) — nutzt dieselbe **vereinigte Quelle** wie das vps-targets-Listing und `resolveVpsTarget`: Env-Map ⊕ dynamische Records (`getTargetRecord(vpsId)` / `listTargetRecords()`), **Env gewinnt bei Kollision**. Findet sich nur ein dynamischer Datensatz, liefert er `{ host, port, targetUser }`; ist `host` veraltet/leer, wird er über `getMachineIp` aufgefrischt (analog AC2/AC4). Erst wenn **weder** Env **noch** dynamische Quelle ein Ziel ergeben, antwortet der Handler weiterhin mit `422 "Unbekannter VPS: <id>"`. Damit ist ein nur-dynamisch angelegter VPS end-to-end deploybar/undeploybar/listbar. Der Handler benötigt dafür Zugriff auf die `vpsRegistry` (heute erhält `deploymentsRouter` nur die statische `vpsTargets`-Map — die Factory-Signatur/Wiring in `src/routers/deployments.js` ist entsprechend zu erweitern). **Audit-First, Rollen-/Identitätsschutz (AC8/AC9 der [[deploy-lifecycle]]) und der AccessGuard-403 bleiben unverändert** — nur die Ziel-**Auflösung** ändert sich, nicht die Mutations-Sicherheit.
...
### Deploy-/Undeploy-/Deployments-Auflösung (v2)
- **AC9** — Die `deploymentsRouter`-Handler **Deploy** (`POST /api/deployments`), **Undeploy** (`DELETE /api/deployments/:vps/:hostname`) und **Deployments-Listing** (`GET /api/deployments`) lösen die `vpsId` über die **vereinigte Quelle** (Env-Map ⊕ dynamische Records, Env gewinnt bei Kollision) auf — **identisch** zu `vps-targets` (AC3) und `resolveVpsTarget` (AC4). Ein **nur dynamisch** angelegter VPS (leere `VPS_TARGETS`-Env, real: `testdevgui`) wird aufgelöst und liefert **kein** `422 "Unbekannter VPS"` mehr; ist `host` veraltet/leer, wird er über `getMachineIp` aufgefrischt. Bei Env↔dynamisch-Kollision gewinnt der Env-Wert. `422 "Unbekannter VPS: <id>"` bleibt **nur** für eine `vpsId`, die in **keiner** Quelle existiert. Audit-First, Rollen-/Identitätsschutz und der AccessGuard-403 bleiben unverändert. (Testbar mit gemocktem Store/Registry/Orchestrator: leere Env + dynamischer `testdevgui`-Datensatz → Deploy/Undeploy/Listing rufen den Orchestrator mit dem aufgelösten `{host,port,targetUser}` statt 422; Kollision → Env-Ziel gewinnt; unbekannte ID → 422; AccessGuard-403 bei fehlender Berechtigung weiterhin vor der Auflösung.)

### Sicherheit (Floor, hart)
- **AC8** — Der Ziel-Datensatz und alle Auflösungs-Pfade (inkl. der `deploymentsRouter`-Handler aus AC9) leaken **niemals** SSH-Private-Key-Material oder Tunnel-Token in Response, Log, Audit, WS-Stream, Argv, URL oder Frontend-Bundle; der Private-Key bleibt store-intern und transient pro Aufruf (ADR-008), das Tunnel-Token bleibt verschlüsselt at rest (ADR-007). `vps-targets` exponiert weiterhin **nur IDs**; die `422`-Fehlermeldung enthält nur die `vpsId`, **kein** host/targetUser/Key/Token. (Testbar: Response/Log/Argv secret-frei; Grep auf Key-/Token-Leak im neuen Pfad.)
...
- **`deploymentsRouter`-vpsId-Auflösung (v2):** Die drei Handler ersetzen das reine `vpsTargets.get(vpsId)` durch eine **vereinigte Auflösung** `resolve(vpsId) → VpsTarget | undefined` = Env-Map ⊕ dynamische Records (`vpsRegistry.getTargetRecord(vpsId)` bzw. `listTargetRecords()`), **Env gewinnt bei Kollision**, host ggf. via `getMachineIp` aufgefrischt. `undefined` → unverändert `422 { error: "Unbekannter VPS: <vpsId>" }`. **`POST /api/deployments`** (Body `{ image, vps, hostname, tunnelId }`), **`DELETE /api/deployments/:vps/:hostname`** (Body `{ confirm, tunnelId }`) und **`GET /api/deployments?vps=&tunnelId=`** behalten Signatur, Audit-First, Authz und Statuscodes; nur die Ziel-Auflösung wird vereinigt.
- **`deploymentsRouter`-Factory (Wiring):** `deploymentsRouter(...)` erhält künftig die `vpsRegistry` (zusätzlich zur `vpsTargets`-Map); `src/routers/deployments.js` `create(deps)` reicht `vpsRegistry` durch. Optional kapselt ein gemeinsamer Resolver-Helper die Vereinigung, den auch `vps-targets` (AC3) und `resolveVpsTarget` (AC4) nutzen (DRY, eine Auflösungs-Wahrheit).
...
- **(v2)** Deploy/Undeploy/Listing gegen eine `vpsId`, die **nur** dynamisch existiert (leere Env) → aufgelöst, kein 422 (AC9).
- **(v2)** Deploy/Undeploy gegen eine `vpsId`, die in **keiner** Quelle existiert → unverändert `422 "Unbekannter VPS: <id>"` (kein stiller Erfolg, kein Leak).
- **(v2)** `vpsId` in Env **und** dynamisch (Kollision) → Env-Ziel gewinnt in allen Handlern (konsistent zu AC3/AC4).
- **(v2)** Dynamischer Datensatz mit veralteter/leerer host-IP → `getMachineIp`-Refresh wie bei `resolveVpsTarget`; schlägt der Refresh fehl, degradiert der nachgelagerte SSH-Pfad geheimnisfrei (`unreachable`), kein Crash, kein 422 wegen reiner IP-Staleness.

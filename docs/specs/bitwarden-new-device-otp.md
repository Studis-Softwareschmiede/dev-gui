...
9. **Ein langlebiger bw-Prozess über beide Requests (Bugfix, hart):** New-Device-Verification ist **prozess-gebunden** — Bitwarden generiert den E-Mail-Code beim Start eines bestimmten `bw login`-PTY-Prozesses und **invalidiert** ihn, sobald ein **neuer** `bw login`-Prozess startet (er erzeugt dann einen frischen Code). Daher MUSS der eingegebene E-Mail-Code in **demselben** `bw login`-Prozess verarbeitet werden, in dem der New-Device-Prompt erschien. Der bisherige two-shot-Fluss (Request 1 startet+beendet einen Prozess, Request 2 startet einen **zweiten** Prozess) ist deshalb fehlerhaft: der zum ersten Prozess gehörende Code ist im zweiten Prozess **immer** ungültig (`email-otp-invalid`, unabhängig vom eingegebenen Code). Korrektes Verhalten: Request 1 (ohne `emailOtp`) startet **einen** `bw login`-PTY-Prozess, läuft bis zum New-Device-Prompt und **hält den Prozess offen** (zustandsbehaftet, geschlüsselt nach Session/Identität); Request 2 (mit `emailOtp`) schreibt den Code in **denselben** offenen Prozess → Login gelingt. Der offene Prozess unterliegt einem **Timeout** und wird bei Erfolg, terminalem Fehler, Timeout oder Abbruch **sauber beendet/aufgeräumt** (kein verwaister PTY, kein Secret-Echo aus dem PTY-Stream nach außen).
...
- **AC10** — Der E-Mail-OTP-Code wird in **demselben** langlebigen `bw login`-PTY-Prozess verarbeitet, in dem der New-Device-Prompt erschien: Request 1 (ohne `emailOtp`) hält den Prozess nach `email-otp-required` **offen** (zustandsbehaftet, geschlüsselt nach Session/Identität); Request 2 (mit `emailOtp`) schreibt den Code in **denselben** offenen Prozess. Es wird **kein** zweiter `bw login`-Prozess gestartet, um den Code zu prüfen. Bei korrektem Code gelingt der Login (Item lesen → `CredentialStore.unlock` wie AC2). (Testbar: nur **ein** `bw login`-Spawn über den OTP-Zyklus; bei gültigem Code kein `email-otp-invalid`.)
- **AC11** — Robustheit/Cleanup des offenen Prozesses: der offene `bw login`-Prozess unterliegt einem **Timeout** und wird bei Erfolg, terminalem Fehler, Timeout oder Abbruch **beendet und aufgeräumt** (kein verwaister PTY/Zombie, kein gehaltener State-Eintrag). Während der Prozess offen ist, gelangt **kein** PTY-Echo/Secret in Log/Response/WS/Audit (Floor AC7 bleibt erfüllt). Läuft der offene Prozess in den Timeout, liefert ein dann eintreffender Folge-Request einen klassifizierten, geheimnisfreien Fehler (z.B. erneut `email-otp-required` mit frischem Start), **nie** einen Leak.
...
- **Beschaffungs-Komponente ([[bitwarden-master-key-unlock]], erweitert):**
  ...
  - **Prozess-State (single-process, AC10/AC11):** Bei `email-otp-required` hält die Komponente den laufenden `bw login`-PTY-Prozess **offen** und führt einen **internen, geheimnisfreien State-Handle** (geschlüsselt nach Session/Identität; enthält **keine** Login-Daten), über den der Folge-Aufruf mit `emailOtp` denselben Prozess adressiert. Der State-Handle ist transient, timeout-behaftet und wird bei Erfolg/terminalem Fehler/Timeout/Abbruch verworfen. Ein Folge-Aufruf mit `emailOtp` **ohne** passenden offenen Prozess (z.B. nach Timeout) startet **nicht** stillschweigend einen neuen Prüf-Prozess mit dem Alt-Code, sondern liefert einen klassifizierten Fehler (`email-otp-required`/`email-otp-invalid`).
...
- **Two-shot-Prozess-Neustart (Bugfix-Anlass S-128):** Der Code wird in einem **zweiten** `bw login`-Prozess geprüft ⇒ Bitwarden hat den ersten Code bereits invalidiert und einen neuen generiert ⇒ Eingabe stets `email-otp-invalid` unabhängig vom Code. **Verboten** (AC10): Es darf für den OTP-Zyklus nur **ein** `bw login`-Prozess existieren.
- Offener Prozess läuft in den Timeout, bevor der Code kommt ⇒ Prozess wird beendet/aufgeräumt; Folge-Request liefert klassifizierten, geheimnisfreien Fehler (AC11) — **kein** verwaister Prozess, **kein** Leak.
- Zwei parallele Unlock-Versuche derselben Identität ⇒ der State-Handle ist eindeutig geschlüsselt; ein zweiter offener Prozess ersetzt/beendet einen veralteten sauber (kein Prozess-Leak).

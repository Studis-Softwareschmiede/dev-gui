...
/**
 * config-App-Slug (deploy-config-volume-mount AC2/AC7): identischer Zeichensatz wie
 * VpsDockerControl#isValidConfigAppSlug — muss mit a-z0-9 beginnen, dann a-z0-9._-.
 */
...
function validateDeployBody(body) {
  ...
  // deploy-config-volume-mount AC7: optionale config.yaml-Mount-Params.
  // requiresConfig (boolean) schaltet den Mount/das Guard-/Seed-Gate scharf (S-347).
  let requiresConfigVal = false;
  if (requiresConfig !== undefined && requiresConfig !== null) {
    if (typeof requiresConfig !== 'boolean') {
      return { ok: false, error: 'requiresConfig muss ein boolean sein' };
    }
    requiresConfigVal = requiresConfig;
  }

  // configApp: string-Slug (Host-Pfad ~/apps/<configApp>/config.yaml); Zeichensatz/Länge
  // identisch zur VpsDockerControl-seitigen Validierung (config-app-invalid, D3).
  let configAppVal = null;
  if (configApp !== undefined && configApp !== null && configApp !== '') {
    if (typeof configApp !== 'string' || !configApp.trim()) {
      return { ok: false, error: 'configApp muss ein nicht-leerer String sein' };
    }
    const trimmedApp = configApp.trim();
    if (trimmedApp.length > MAX_CONFIG_APP_LEN) {
      return { ok: false, error: `configApp überschreitet Längenlimit (max. ${MAX_CONFIG_APP_LEN} Zeichen)` };
    }
    if (!CONFIG_APP_SLUG_RE.test(trimmedApp)) {
      return { ok: false, error: 'configApp enthält ungültige Zeichen (nur a-z 0-9 . _ - erlaubt, muss mit a-z0-9 beginnen)' };
    }

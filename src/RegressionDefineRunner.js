 ...
 *
 * Reihenfolge-Disziplin (AC15/AC17, Lehre aus dem ersten Umsetzungsversuch):
 * der Datei-Lese-Schritt (`readFile`) UND der Aufräum-Schritt (`unlink`)
 * werden VOLLSTÄNDIG AWAITED, BEVOR der Job auf einen Zustand (terminal ODER
 * `needs-review`) übergeht — kein fire-and-forget dazwischen. Andernfalls
 * bliebe der Job sichtbar auf `running` hängen, obwohl das Ergebnis bereits
 * vorliegt (Async-Race, per CI beobachtet).
 *
...
export class RegressionDefineRunner {
  ...
  async #runRound(jobId, { resume, promptArg, reviewed }) {
    ...
    // terminal/`needs-review` gesetzt wird (kein fire-and-forget, sonst bleibt
    // der Job sichtbar auf `running` hängen — Async-Race, s. Modul-Doku).
    let fileContent;
    try {

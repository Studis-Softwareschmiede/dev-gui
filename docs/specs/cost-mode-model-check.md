...
- **Dispatch-Auslöser reicht die `checkId` ans Frontend (AC4/AC5 — Frontend-Anbindung).** Der manuelle „Board abarbeiten"-Drain (`POST /api/projects/:slug/drain`, [[headless-manual-drain]]) stößt bei der Cost-Mode-Übergabe die Dispatch-Frische-Prüfung an (`CostModeModelCheck.runCheck('dispatch')`) und ergänzt seine bestehende `202`-Antwort bei **erkanntem Drift** um das **optionale** Feld `costModeCheckId` (`202 { drainId, costModeCheckId? }`). Das Frontend pollt damit `GET /api/cost-mode/check/:checkId` und zeigt die nicht-modale „Modell veraltet"-Meldung + Vorher/Nachher-Übersicht. **Kein Drift / bereits laufender Curator (`skipped`) / fehlende Boundary → kein Feld** (stiller Normalfall). Das Feld ist **nicht-blockierend** (der Drain wird VOR der Prüfung fire-and-forget gestartet, AC5) und rein additiv — der `drainId`-Vertrag von [[headless-manual-drain]] bleibt unverändert. Der **Nacht-Drain** ([[headless-parallel-drain]]) stößt dieselbe Prüfung headless an (fire-and-forget, ohne Frontend-Rückkanal — keine GUI-Sitzung).

...
9. Ein Button **„Board abarbeiten"** löst die **ProjectDrain-Engine** aus (über `POST /api/projects/:slug/drain`, [[taktgeber-nachtwaechter]] AC12) — die intern `/agent-flow:flow` wiederholt über den bestehenden `POST /api/command`-Kanal anstößt, bis das Board des Projekts keine offene Drain-Ziel-Story mehr hat. Das ist der „starten und schlafen gehen"-Teil; jeder Lauf erscheint live im Terminal. Bei bereits laufendem Job ist der Button — wie alle Trigger — deaktiviert (bestehendes globales Lock-Modell, [[flow-trigger]] AC3).
...
- **AC8** — Der Button „Board abarbeiten" löst die ProjectDrain-Engine über `POST /api/projects/:slug/drain` aus ([[taktgeber-nachtwaechter]] AC12 — intern wiederholte `/agent-flow:flow`-Anstöße über `POST /api/command`) und ist bei aktivem Job (Session `busy`) deaktiviert; der Kill-Switch aus [[flow-trigger]] bleibt wirksam.
...
## Abhängigkeiten
- [[flow-trigger]] (Command-Kanal, Allowlist, Lock, Audit, Cost-Mode) · [[terminal-frontend]] (Live-Ausgabe, Pane-Wechsel) · [[access-and-guardrails]] (AccessGuard, Audit-First) · [[taktgeber-nachtwaechter]] (ProjectDrain-Engine hinter „Board abarbeiten", AC12).

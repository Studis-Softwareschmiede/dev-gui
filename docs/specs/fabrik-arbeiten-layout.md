...
2. Die **Live-Ausgabe der verbleibenden interaktiven PTY-Befehle** (adopt/preview/train/new-project + Kill-Switch, [[headless-manual-drain]] AC8) bleibt erreichbar — entweder über eine **on-demand/aufklappbare** Terminal-Fläche, die beim Auslösen eines interaktiven Befehls erscheint, **oder** über die bestehende freie „Terminal"-Fläche. **Kein** Verlust der interaktiven Ausgabe; der interaktive PTY-Pfad bleibt funktional unverändert.
...
- **AC2** — Die Live-Ausgabe der verbleibenden interaktiven PTY-Befehle (adopt/preview/train/new-project + Kill) bleibt **erreichbar** (on-demand/aufklappbare Fläche **oder** freie „Terminal"-Fläche); nach Auslösen eines solchen Befehls ist dessen Ausgabe für den Nutzer sichtbar. Der interaktive PTY-Pfad (`PtyManager`/`CommandService`) ist funktional unverändert. (Testbar: nach Trigger eines interaktiven Befehls erscheint/aktualisiert sich die Ausgabefläche; kein Verlust der Verbindung.)
...
## Verträge
- Konsumiert das bestehende Cockpit-Gerüst ([[projekt-cockpit-navigation]] `CockpitView.jsx`, `FactoryView.jsx`) + `TriggerPanel.jsx` (verschlankt, [[headless-manual-drain]] AC8) + `Terminal.jsx` (für die on-demand/freie Ausgabefläche, falls gewählt).
...
- Interaktiver Befehl ausgelöst, on-demand-Fläche gewählt → sie öffnet sich und zeigt die PTY-Ausgabe; wird sie geschlossen, läuft die Session im Backend unbeeinflusst weiter (kein Kill durch UI-Schließen).

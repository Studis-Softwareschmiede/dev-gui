...
## Verhalten
1. Das Backend aggregiert auf Anfrage live: Org-Projekte (Repos ≠ `agent-flow`), je Projekt die Zahl offener Board-Items, den Status des letzten CI-Laufs und die laufenden Preview-Container (Name/Port→URL/Status).
...
- **AC1** — `GET /api/status` liefert pro Projekt: `name`, `openItems` (Anzahl offener Board-Items), `lastCi` (`success|failure|in_progress|none`), und global `previews` (Liste laufender Preview-Container mit `name`, `url`, `status`).
...
## Verträge
- `GET /api/status` → `200 {projects:[{name, openItems, lastCi}], previews:[{name, url, status}]}`.

 ...
 *
 * Benachrichtigungen (ntfy) — push-notifications S-183 AC3/AC10:
 *   AC3  — Sektion „Benachrichtigungen (ntfy)": Ein/Aus-Schalter, Server-URL (Default https://ntfy.sh),
 *           Topic, Priorität, Token write-only via Credential-Pfad (gesetzt/nicht gesetzt, NIE Klartext),
 *           Ereignis-Checkboxen (story_done/story_blocked/feature_done).
 *           Gespeicherte Werte beim Laden angezeigt (GET /api/settings/notifications).
 *           Test-Benachrichtigung via POST /api/settings/notifications/test.
 *   AC10 — Token NIE im Klartext im State/DOM; nur has_token-Bool aus GET-Response; Token nur über
 *           PUT /api/settings/credentials/notifications/ntfy_token (bestehender Credential-Pfad).
 *
 ...
 *
 * Obsidian-Vault-Pfad (obsidian-vault-config S-247 — AC1, UI-Anteil):
 *   AC1 — Eigene Sektion „Obsidian" (Spec-Entscheidung A2, nicht in der GitHub-Sektion) zeigt
 *         den konfigurierten Vault-Pfad + Zustand (konfiguriert / nicht konfiguriert); erlaubt
 *         setzen/ändern (PUT /api/settings/obsidian-vault-path, Body {path}) und
 *         löschen/zurücksetzen (DELETE). mountRoot (falls vom Backend geliefert) als Hinweis.
 *         422-Validierungsfehler (nicht existent / kein Verzeichnis / nicht lesbar /
 *         „Projekte" fehlt / Traversal) und 403 (keine Berechtigung) feldzugeordnet angezeigt;
 *         bisheriger Wert bleibt bei Fehler sichtbar. Muster gespiegelt von
 *         `WorkspacePathSection` (workspace-path-config).
 ...
 */
async function fetchNotificationSettings(fetchImpl) {
  const fn = fetchImpl ?? globalThis.fetch.bind(globalThis);
 ...
 * @returns {Promise<object>}
 */
async function putNotificationSettings(settings, fetchImpl) {
  const fn = fetchImpl ?? globalThis.fetch.bind(globalThis);
  const res = await fn('/api/settings/notifications', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
 ...
 */
async function postNotificationTest(fetchImpl) {
  const fn = fetchImpl ?? globalThis.fetch.bind(globalThis);
 ...
 * }} props
 */
function NotificationSection({ notificationsCredMeta, onCredSaved, fetchFn }) {
  // Form-State
...
/** Styles für NotificationSection (S-183 AC3). */
const notifStyles = {
  fieldRow: {
    display: 'flex',
    alignItems: 'center',
  ...
  return (
    <main style={styles.view} aria-label="Einstellungen-Ansicht">
      <div style={styles.inner}>
        ...
        {/* Benachrichtigungen (ntfy) — S-183 AC3 */}
        ...
        {/* Obsidian — obsidian-vault-config AC1 (UI-Anteil, S-247), Spec-Entscheidung A2 (eigene Sektion) */}

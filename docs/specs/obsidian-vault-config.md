...
- **Env `OBSIDIAN_PROJEKTE_SUBDIR`** (optional, S-330/v2): konfiguriert den Projekt-Unterordner als Pfad-Segment (Mehrebenen erlaubt, z. B. `300 Projekte/Studis Softwareschmiede`). Ist sie ungesetzt/leer → Default „Projekte" (Rückwärtskompatibilität). Deploy-Zeit-Konfiguration, analog `OBSIDIAN_VAULT_DIR`.

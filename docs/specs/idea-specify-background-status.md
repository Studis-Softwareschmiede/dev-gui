...
- `start(...)` registriert den Job **synchron** (keyed by `ideaStoryId` **und** `jobId`) vor dem Spawn; lehnt Start ab, wenn für die Idee bereits ein Job `running` ist (`409`).
- `jobsForProject(projectSlug) → { [ideaStoryId]: {...} }` (nur nicht-`done`) und `statusForIdea(projectSlug, ideaStoryId) → {...}|null` speisen die Read-Endpunkte.

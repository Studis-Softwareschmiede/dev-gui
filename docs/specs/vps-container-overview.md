...
- **AC11b** *(v3)* — **managed-Remove ist fail-closed bzgl. Route:** ist der Container **managed** und die `tunnelId` **nicht auflösbar**, bricht das Entfernen mit 422 `tunnel-not-found` ab — **kein** `rm`, **kein** Cloudflare-Schritt, der Container bleibt unangetastet. Ein **stiller Rückfall auf reines `VpsDockerControl.rm`** ist **untersagt**: er löscht den Container und lässt **Tunnel-Route + DNS-CNAME verwaist** zurück — ohne Fehlermeldung, ohne dass der Betreiber es bemerkt. **Unmanaged** Container (kein Hostname-Label) bleiben unverändert beim reinen `rm`-Pfad — sie haben keine Route. Die tunnelId-Auflösung ist **exakt dieselbe** wie in [[container-image-update]] AC13 (Target-Record der `{provider, serverId}`-Registrierung) — **eine** Implementierung, **kein** zweiter Weg (Grep-prüfbar). (Testbar: managed + auflösbare tunnelId → `undeploy` inkl. `removeRoute`; managed + **keine** tunnelId → 422 `tunnel-not-found`, `rm`-Mock **nie** aufgerufen; unmanaged → `rm`, kein Cloudflare-Schritt.)

> **Befund (2026-07-15, am Code verifiziert):** der managed-Remove nutzte dieselbe defekte Auflösung wie der Update-Knopf ([[container-image-update]] AC13), **tolerierte** deren `null`-Ergebnis aber als Fallback auf reines `docker rm`. Da die Auflösung in Produktion **immer** `null` lieferte, hat **jeder** managed-Remove über dev-gui den Container gelöscht und Route + DNS verwaist zurückgelassen — **leise**. Genau das hat die Bruchstelle verdeckt: der Remove degradierte still, während das Update laut fail-closed abbrach.
...
- **Entfernen managed Container ohne auflösbare `tunnelId`** (VPS ohne Target-Record bzw. ohne angelegten Tunnel) → 422 `tunnel-not-found`, **kein** `rm` (AC11b). **Bewusste Verschärfung gegenüber dem Alt-Verhalten:** bisher wurde der Container in diesem Fall still ohne Route-Abbau gelöscht. Lieber ein sichtbarer Abbruch als ein leise verwaister DNS-Eintrag. Wer einen solchen Container dennoch loswerden muss, nutzt den Weg über die Deployments-Ansicht bzw. räumt die Registrierung auf.
...
- **Kein stiller Teil-Abbau (hart, AC11b):** Degradation ist beim **Lesen** richtig (Übersicht bleibt nutzbar) und beim **Abbau einer managed Ressource falsch**. Ein Remove baut Container **und** Route **gemeinsam** ab oder **gar nicht** — ein „Container weg, Route bleibt"-Ergebnis ist nie ein zulässiger Fallback, weil es unbemerkt bleibt und den Betreiber mit verwaisten DNS-Einträgen zurücklässt.

...
## Verträge
- **Rotation-Operation (`POST /api/settings/credential-rotate`, S-083 Kern — gelandet):** hinter Access + `CRED_ADMIN_EMAILS`, Audit-First. Request-Body: `{ newKey: string }` — der Kern-Endpunkt nimmt den neuen Key direkt aus dem Body entgegen; eine Beschaffung des neuen Keys über Bitwarden ist kein Bestandteil dieses Endpunkts. Response: `{ ok: true, swapped: true }` bzw. `{ ok: false, reason, swapped }` — meldet nur Erfolg/Phase, **nie** einen Key. Das `swapped`-Feld ist die Andock-Naht für die Bitwarden-Archiv-/Backup-Folge-Story (S-342): `swapped:true` signalisiert, dass der Store bereits ausschließlich mit dem neuen Key arbeitet (unabhängig vom `.env`-Persistenz-Ausgang), also der Zeitpunkt für Archivierung/Backup erreicht ist.
- **`CredentialStore`-Erweiterung (`rotate(newKey)`, S-083 Kern — gelandet):** `Promise<{ ok: true, swapped: true } | { ok: false, reason, swapped: boolean }>` — führt (a)–(d) aus (bei einem Store ohne verschlüsselte Einträge entfällt die Datei-Rotation trivial, der Key wird trotzdem aktiviert); gibt **nie** einen Key zurück; im Fehlerfall sanitisierter `reason` ohne Wert (`empty-key`|`invalid-key-format`|`no-master-key`|`same-key`|`decrypt-failed`|`encrypt-failed`|`verification-failed`|`swap-failed`|`persist-failed`). **v2-Präzisierung (S-342, AC6/AC12, additiv):** sobald der neue Key aktiv ist (Swap bzw. die datei-lose Aktivierung), trägt der Rückgabewert zusätzlich ein `backup`-Feld (Ergebnis des bestehenden Backup-Hooks, [[credential-backup]]-Shape `{local, offHost, ...}`) — rein additiv zum bestehenden `{ok, swapped}`-Vertrag, kein Bruch für Aufrufer die das Feld ignorieren.
- **Bitwarden-Archiv (v2, verbindlich):** Der neue Key wird der **aktive** Wert des Items `dev-gui-master-key`; der bisherige Key wird **datiert** ins Custom-Feld **„Schlüssel-Archiv"** desselben Items geschrieben (append, nie überschreiben) — kein Löschen, solange alte S3/R2-Backups ihn brauchen (AC11). (Der v1-Vorschlag `…-current`/`…-previous`-Items wird durch dieses Archiv-Feld ersetzt.) Die Archivierung läuft über die Beschaffungs-Boundary [[bitwarden-master-key-unlock]] (`BitwardenMasterKeyService#archiveRotatedKey`, dieselbe interaktive E-Mail+Passwort(+2FA/E-Mail-OTP)-Login-Technik wie `acquireMasterKey`/`createMasterKey`) — der jeweils aktuelle Bitwarden-Item-Wert (nicht der `CredentialStore`-interne Key) gilt als „bisheriger Key" (Bitwarden bleibt Source of Truth).
- **Backup-Artefakt:** frische, mit dem neuen Key lesbare Kopie von `secrets.enc.json`; Ablageort beim Umsetzungs-Item (Backup/Restore-Verzahnung war in ADR-014 bewusst out-of-scope und wird hier als Folge zusammengeführt). Ausgelöst additiv innerhalb von `CredentialStore#rotate()` selbst (derselbe lock-geschützte Schreibpfad, [[credential-backup]] §Behavior 1 „einziger Hook").
- **Permanente Entsorgung (AC5, v2-Präzisierung):** im Archiv-Modell gibt es keine separaten Bitwarden-Items mehr zu löschen (nur das eine Item `dev-gui-master-key` mit wachsendem Archiv-Feld) — „permanentes Löschen" bedeutet hier das explizit bestätigte, vollständige Leeren des Custom-Felds „Schlüssel-Archiv" (`BitwardenMasterKeyService#discardArchivedKeys`), NIE automatisch im Rotations-Flow. Der in AC5 genannte „Trash-Check" bezieht sich auf das v1-Item-Lösch-Modell und ist im Feld-Wipe-Modell nicht sinngleich anwendbar (SPEC-LÜCKE — der Owner sollte klären, ob eine feingranulare Einzel-Eintrags-Entsorgung statt eines Voll-Wipes gewünscht ist; diese Implementierung wählt den vollständigen Wipe als konservative, klar auditierbare Interpretation).
...
- Voll-automatische zeitgesteuerte Rotation — Rotation ist ausschließlich on-demand.

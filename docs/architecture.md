...
- **VpsProviderRegistry / VpsProvider-Adapter** *(neu, Multi-Provider — Boundary für Cloud-Provider-APIs; **ADR vom `architekt` zu fixieren**)* — einziger Ort, der Provider-APIs (Hetzner, IONOS, Hostinger) anspricht; provider-agnostischer `VpsProvider`-Vertrag (list/start/stop/create + Capability-Flags), je Provider ein Adapter. Endpunkte `GET /api/vps/providers`, `GET /api/vps/machines`, `POST /api/vps/machines/{provider}` (Create-from-scratch mit cloud-init-user-data), `POST /api/vps/machines/{provider}/{serverId}/start|stop`. Provider-API-Token store-intern aus dem `CredentialStore` (`vps/<provider>_api_token`), transient pro Aufruf, **nie** in Response/Log/Audit/WS/Argv. Read-Aggregation **live** (kein Maschinen-Store, ADR-005-Linie), pro Provider degradierend. Create übergibt cloud-init-user-data ([[vps-cloud-init-setup]]) + root-/alex-Public-Keys ([[vps-ssh-key-assignment]]). Server-mutierende Aktionen auditiert (Audit-First) + identitäts-/rollengeschützt (gleiche `CRED_ADMIN_EMAILS`-Logik wie ADR-007). **Touchpoints für `architekt`/`coder`:** Provider-SDK-/REST-Wahl je Provider (eigene Auth/Produktlinien/Lifecycle-Semantik; IONOS/Hostinger ggf. neuere/abweichende APIs; nicht unterstützte Lifecycle-Aktionen als `unsupported` ausweisen, nicht erzwingen), cloud-init-Erzeugung, Token-Injektion ohne Persistenz. Specs: [[vps-provider-boundary]], [[vps-cloud-init-setup]], [[vps-ssh-key-assignment]], [[view-vps]]; **Rebuild/Backup vertagt** ([[vps-rebuild-backup]], Platzhalter).

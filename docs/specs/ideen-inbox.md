...
- **Besprechung während der Taktgeber (`ProjectDrain`) dasselbe Projekt drained** → `409`, auch wenn `CommandService` zwischen zwei `/flow`-Runden gerade kurz idle ist: der Besprechungs-Start hält kurz (Check+PTY-Write) das projektweite `ProjectJobLock` (`[[taktgeber-nachtwaechter]]` AC6/AC7, geteilt mit `ProjectDrain`) — schließt die Naht zwischen manuellem Seed und autonomem Weiter-Drain derselben PTY-Session. Symmetrisch: zwei gleichzeitige Besprechungs-Starts fürs selbe Projekt sind ebenso gegen `409` abgesichert.

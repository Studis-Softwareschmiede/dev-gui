...
> **⟶ Ergänzt (2026-07-07, [[drain-origin-progress-sync]]):** Bei Projekten mit `merge_policy: pr` landen `/flow`-Board-Flips per PR auf `origin/<default_branch>` — der lokale Working-Tree-Checkout, den `BoardAggregator`/`ProjectDrain` bis dahin ausschließlich lasen, kann dann hinter `origin` zurückliegen (verifizierter Vorfall 2026-07-07). Die Snapshot-Quelle für Fortschritt/Eskalation/Abschlussbericht ist seither **origin-bewusst**: vor jeder Bewertung führt der Taktgeber ein read-only `git fetch origin` aus; ist der lokale `HEAD` ein echter Vorfahr des Remote-Tracking-Refs (ancestry-basiert), wird der Board-Snapshot READ-ONLY aus dem `origin`-Ref gelesen statt aus dem Working-Tree; sonst (u.a. `merge_policy: direct`, wo `HEAD ≥ origin` gilt) bleibt der Working-Tree unverändert die Quelle — **keine** Regression für `direct`-Projekte. Eskalation (`setBlocked`) setzt zusätzlich einen **erfolgreichen Fetch** voraus (`verified`-Gate) — bei Fetch-Fehler kein `Blocked`-Write, kein Zähler-Increment, nur Retry. **Keine** working-tree-mutierende Git-Operation (`pull`/`merge`/`checkout`/`reset`/`clean`/`stash`) — ausschließlich `fetch` (Refs) + read-only Objekt-Lese. Die Drain-/Abbruch-/Eskalations-**Logik** unten (Regel 1–5, AC1–AC5) bleibt **unverändert** — nur die Snapshot-**Quelle** + das Eskalations-**Gate** kommen hinzu. Detail-Spec: [[drain-origin-progress-sync]] (AC1–AC7).
...
- **AC4** — Eskalation: nach `escalationAttempts` (Default 3) aufeinanderfolgenden Läufen **ohne** jede **Drain-Ziel**-Story-Statusänderung setzt der Taktgeber die am längsten unbewegte Drain-Ziel-Story selbst auf `Blocked` mit `blocked_reason: "Taktgeber: Nx kein Fortschritt"` (N = `escalationAttempts`) und setzt den kein-Fortschritt-Zähler zurück. Nicht-ready `To Do` und `Idee` werden nie eskaliert. Garantierte Konvergenz der Abbruch-Regel. *(4)*
  - **⟶ Ergänzt (2026-07-07, [[drain-origin-progress-sync]] AC4):** Eskalation (`setBlocked`) setzt zusätzlich einen **verifiziert-aktuellen** Snapshot voraus (erfolgreiches read-only `git fetch origin`). Bei Fetch-Fehler wird in dieser Runde **kein** `Blocked` geschrieben und der kein-Fortschritt-Zähler **nicht** hochgezählt (Log, Retry im nächsten Tick) — verhindert Eskalation auf einem stalen lokalen Checkout bei `merge_policy: pr`.
  - **⟶ Ergänzt (2026-07-26, [[drain-escalation-effectiveness]] AC1–AC5, Vorfall „Taktgeber-Endlosschleife"):** Eine Eskalation zählt nur dann als Fortschritt, wenn ihr `Blocked`-Übergang in einem **Folge-Snapshot derselben Quelle** sichtbar wird (Wirksamkeits-Check) — der frühere unbedingte Sicherheitsgürtel-Zähler-Reset nach `#escalate` entfällt. Bei Snapshot-Quelle **`origin-ref`** wird der `setBlocked`-Write vollständig **unterlassen** (unsichtbar + Dirty-Leiche; Audit). Je Drain-Session wird jede Story höchstens **einmal** (versucht) eskaliert (Per-Story-Cap); sind alle Ziele versucht und wirkungslos, endet der Drain terminal mit `reason: 'escalation-ineffective'`.
- **AC5** — Fortschritt = jede Story-Statusänderung zwischen zwei Board-Scans **oder** der Übergang einer `To Do`-Story von `ready==false` nach `ready==true` (freigeschaltet durch einen fertigen Vorgänger); bei Fortschritt wird der kein-Fortschritt-Zähler zurückgesetzt (Eskalation nur bei **aufeinanderfolgenden** fortschrittslosen Läufen). *(5)*
  - **⟶ Ergänzt (2026-07-07, [[drain-origin-progress-sync]] AC2/AC3/AC5):** „Zwei Board-Scans" meint seither zwei Snapshots derselben, je Runde ancestry-basiert gewählten Quelle (Working-Tree ODER `origin`-Ref bei verifiziert-vorauslaufendem `origin`) — eine per PR frisch auf `origin` gelandete Story zählt dadurch bei `merge_policy: pr` korrekt als Fortschritt, auch wenn der lokale Working-Tree-Checkout noch nicht nachgezogen wurde. Ein dirty Working-Tree wird dabei nie verändert und löst nie eine Eskalation auf stalem Stand aus.
...
- **AC12** — Der manuelle Knopf „Board abarbeiten" nutzt dieselbe `ProjectDrain`-Engine: draint das aktuell geöffnete Projekt **sofort** (ignoriert das Nachtfenster), Parallelität 1. *(6)*
  - **⟶ Superseded/ergänzt (2026-07-01, [[headless-manual-drain]] / ADR-017):** Der **Ausführungspfad** dieses Knopfs ist seither **headless** (eigene, headless verdrahtete `ProjectDrain`/`HeadlessFlowRunner`-Instanz + eigenes `ProjectJobLock`, `claude -p '/agent-flow:flow'` je Drain-Runde statt PTY-`CommandService`) — die frühere implizite Annahme „interaktiv/PTV" gilt **nicht** mehr. „sofort / aktuell geöffnetes Projekt / Parallelität 1" und die gesamte `ProjectDrain`-Engine-Logik bleiben **unverändert**; hinzu kommt ein optionaler Cost-Mode (`--cost <mode>`) direkt am Knopf.
...
## Abhängigkeiten
- [[headless-parallel-drain]] (headless `claude -p`-Ausführungspfad für echte Nacht-Parallelität, löst `command-channel-busy` im Headless-Modus auf) · [[flow-trigger]] (CommandService, `POST /api/command`, Allowlist) · [[autonome-board-abarbeitung]] (Blocked-statt-Raten, „Board abarbeiten"-Knopf) · [[board-abarbeitungs-strategie]] (Parallelität/Worktrees in agent-flow) · [[ideen-inbox]] (Status `Idee` als explizites Nicht-Drain-Ziel, AC3) · [[push-notifications]] (NotificationSettingsStore-Muster) · `BoardAggregator` + `computeStoryReadyStatus` (ready-Regel) · `PtySessionRegistry` · `ReconciliationJob` (Scheduler-Muster) · `AuditStore` · [[drain-origin-progress-sync]] (origin-basierte, mutationsfreie Snapshot-Quelle + Eskalations-Gate bei `merge_policy: pr`, ergänzt AC4/AC5, s.o.).
